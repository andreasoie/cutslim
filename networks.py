...
def init_net(net, init_type="normal", init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True):
    ...
    """
    if len(gpu_ids) > 0:  # You should use your network on GPUs if there's at least one available.
        device = torch.device("cuda:{}".format(gpu_ids[0]))
        net.to(device)  # Move the network to the GPU before wrapping it in DataParallel.
        if len(gpu_ids) > 1:
...
class Decoder_all(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm="batch", activ="relu", pad_type="zero", nz=0):
        ...
        setattr(
            self, "block_{:d}".format(self.n_blocks), Conv2dBlock(dim + nz, output_dim, 7, 1, 3, norm="none", activation="tanh", pad_type="reflect")
        )
        self.n_blocks += 1
    ...
    def __init__(
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model += [
                    Upsample(ngf * mult),
                    nn.Conv2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=1, padding=1, bias=use_bias),  # output_padding=1,
                    norm_layer(int(ngf * mult / 2)),
                    nn.ReLU(True),
                ]
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(
        self, outer_nc, inner_nc, input_nc=None, submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False
    ):
